  ...
  return (
    <div ref={containerRef}>
      ...
      <div className="progress-rail hidden md:block">
        <motion.div style={{ width: '100%', background: '#B8862A', height: railHeight }} />
      </div>
      ...
      <section style={{ background: '#1E2A4A', padding: '160px 24px 100px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', right: '-2%', bottom: '-5%', pointerEvents: 'none' }} aria-hidden="true">
          <span className="font-display font-bold select-none" style={{ fontSize: 'clamp(80px, 15vw, 220px)', color: 'rgba(184,134,42,0.05)', letterSpacing: '-0.04em', lineHeight: 1 }}>
            {cs.platform}
          </span>
        </div>
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}>
            ...
            <div className="flex flex-wrap gap-3 mt-8">
              {cs.metrics.map(m => (
                <div key={m.label} style={{ background: 'rgba(184,134,42,0.1)', border: '1px solid rgba(184,134,42,0.25)', borderRadius: '9999px', padding: '8px 20px' }}>
                  <span className="font-display font-bold" style={{ fontSize: '16px', color: '#B8862A' }}>{m.value}</span>
      ...
      <section style={{ background: '#FDFBF7', padding: '100px 24px 120px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            ...
            return sectionLabels.map(({ key, label }, i) => {
              ...
              return (
                <div key={key}>
                  ...
                  >
                    <span className="font-body font-semibold uppercase tracking-[0.15em]" style={{ fontSize: '11px', color: '#B8862A' }}>{label}</span>
          ...
          <motion.blockquote
            ...
            style={{ margin: '0 0 72px', padding: '40px 48px', background: '#1E2A4A', borderRadius: '16px', borderLeft: '4px solid #B8862A' }}
          ...
          >
            ...
            <cite className="font-body mt-4 block" style={{ fontSize: '13px', color: '#B8862A', fontStyle: 'normal' }}>
              — {cs.pullQuoteAttrib}
            </cite>
          </motion.blockquote>
