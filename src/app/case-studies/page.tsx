      ...
      <section style={{ background: '#1E2A4A', padding: '160px 24px 100px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 3 }}>
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}>
            <span className="font-body font-semibold uppercase tracking-[0.15em]" style={{ fontSize: '12px', color: '#C9A96E' }}>CODED WORK</span>
            ...
            {([['all', 'All Operations'], ['assan', 'ASSAN Operations'], ['iris', 'IRIS Optimizations']] as [Filter, string][]).map(([key, label]) => (
              <button
                ...
                style={{
                  ...
                  background: filter === key ? '#C9A96E' : 'transparent',
                  borderColor: filter === key ? '#C9A96E' : 'rgba(43,58,103,0.2)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: '20px' }}>
            <AnimatePresence mode="popLayout">
              {filtered.map((cs, i) => (
                ...
                >
                  {/* Ghost numeral */}
                  <span aria-hidden="true" className="font-display font-bold select-none pointer-events-none absolute" style={{ fontSize: 'clamp(70px, 9vw, 110px)', color: 'rgba(201,169,110,0.1)', top: '-8px', right: '-6px', lineHeight: 1 }}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-5">
                    {cs.metrics.slice(0, 2).map(m => (
                      <div key={m.label} style={{ background: 'rgba(201,169,110,0.1)', border: '1px solid rgba(201,169,110,0.2)', borderRadius: '9999px', padding: '4px 12px' }}>
                        <span className="font-display font-bold" style={{ fontSize: '13px', color: '#C9A96E' }}>{m.value}</span>
                        <span className="font-body ml-1" style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)' }}>{m.label}</span>
                      </div>
                    ))}
                  </div>

                  <Link href={`/case-studies/${cs.slug}`} className="font-body font-semibold no-underline mt-6 inline-flex items-center gap-2" style={{ fontSize: '14px', color: '#C9A96E' }}>
                    View Operation
